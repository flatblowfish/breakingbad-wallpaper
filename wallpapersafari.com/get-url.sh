...
###########################################################
#
# https://cdn.wallpapersafari.com/24/69/WBitoA.jpg
#
cat download.html | grep "img-fluid galimg" | gawk 'BEGIN{FS="\""} {print ""$6}' | sort | uniq | wc -l > url-count.txt
#
cat download.html | grep "img-fluid galimg" | gawk 'BEGIN{FS="\""} {print ""$6}' | sort | uniq > url.txt
###########################################################
# 3. 下载文件
###########################################################
#
